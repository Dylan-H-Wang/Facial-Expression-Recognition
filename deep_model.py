...
class VggFace(nn.Module):
    def __init__(self, pretrain=True, path="./models/vgg_face.t7"):
    ...
    def forward(self, x):
        ...
        if self.sparse:
            self.fc_6.weight.data.masked_scatter_(
                self.masks[0], torch.zeros_like(self.fc_6.weight.data)
            )
        x = F.relu(self.fc_6(x))
        x = self.dropout_1(x)

        if self.sparse:
            self.fc_7.weight.data.masked_scatter_(
                self.masks[1], torch.zeros_like(self.fc_7.weight.data)
            )
        x = F.relu(self.fc_7(x))
        x = self.dropout_2(x)

        if self.sparse:
            self.fc_8.weight.data.masked_scatter_(
                self.masks[2], torch.zeros_like(self.fc_8.weight.data)
            )
        x = self.fc_8(x)
...
class Vggf(nn.Module):
    def __init__(self, pretrain=True, path="./models/vgg_f.pth"):
    ...
    def forward(self, x):
        ...
        if self.sparse:
            self.conv3.weight.data.masked_scatter_(
                self.masks[0], torch.zeros_like(self.conv3.weight.data)
            )
        x = F.relu(self.conv3(x))

        if self.sparse:
            self.conv4.weight.data.masked_scatter_(
                self.masks[1], torch.zeros_like(self.conv4.weight.data)
            )
        x = F.relu(self.conv4(x))
        x = self.dropout1(x)

        if self.sparse:
            self.conv5.weight.data.masked_scatter_(
                self.masks[2], torch.zeros_like(self.conv5.weight.data)
            )
        ...
        if self.sparse:
            self.fc6.weight.data.masked_scatter_(
                self.masks[3], torch.zeros_like(self.fc6.weight.data)
            )
        x = F.relu(self.fc6(x))
        x = self.dropout3(x)

        if self.sparse:
            self.fc7.weight.data.masked_scatter_(
                self.masks[4], torch.zeros_like(self.fc7.weight.data)
            )
        x = F.relu(self.fc7(x))
        x = self.dropout4(x)

        if self.sparse:
            self.fc8.weight.data.masked_scatter_(
                self.masks[5], torch.zeros_like(self.fc8.weight.data)
            )
        x = F.relu(self.fc8(x))
